...
const createNewCarrier = (app) => {
  ...
  app.post("/shipping_callback", async (req, res) => {
    ...
    const destination_state_code = data.rate.destination.province 
      ...
      //origin information
      const Origin_CO = CountryObj.countries.find(({code})=>code === origin_country_code);
    ...
    const Destination_CO =  CountryObj.countries.find(({code})=>code === destination_country_code);
    ...
    if(Dest_State_SO.length){
      const sn = Dest_State_SO.find(({code})=> code === destination_state_code);
     ...
     */
      const calculateFee =(sendbox_fee, dbObj) =>{
        ...
        if(adjust_fee === "flat"){
          ...
          result = (sendbox_fee + (sendbox_fee * dbObj?.increasePrecentage)/100 ) * 100;
        }
        else if(adjust_fee === "decrease") {
          result = (sendbox_fee - (sendbox_fee * dbObj?.decreasePrecentage)/100 ) * 100;
        }
        else{
      ...
      try {
          const {data} = await axios.post(`${process.env.BASE_URL}/shipping/shipment_delivery_quote`, quotesPayload, {
            headers:{
          ...
          if(dbObj.activateFreeShipping === true && dbObj.spendLimit === allPrices){
