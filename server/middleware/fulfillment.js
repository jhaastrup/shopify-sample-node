...
const fulfillmentService = (app) => {
    ...
    if (notifictionResponse.kind == "FULFILLMENT_REQUEST") {
        ...
        for (const node in obj) {
          obj[node].forEach((itemObj) => {
            for (const item in itemObj) {
              ...
              _item = {
                name: singleItem.lineItem.name,
                sku: singleItem.lineItem.sku,
                quantity: singleItem.remainingQuantity,
                description: "",
              };
              allItems.push(_item);
              console.log(_item);
      ...
      try {
        const { data } = await axios.post(
          `${process.env.BASE_URL}/warehouse/order_request`,
          orderPayload,
          {
            headers: {
    ...
    if (response.status == "assign_to_warehouse_agent") {
      ...
      const result = await client.query({
        data: {
          ...
          variables: {
            id: _order_id,
            message: "Order has been assigned to Warehouse Agent.",
          },
        },
      });
  ...
  app.post("/create_product", async (req, res) => {
    ...
    if (ful_service == "fulfillment-by-sendbox") {
      ...
      try {
        const { data } = await axios.post(
          `${process.env.BASE_URL}/warehouse/product`,
          productPayload,
          {
            headers: {
              "Access-Token": `Basic ${authToken}`,
  ...
  app.post("/update_inventory", async (req, res) => {
    ...
    const result = await client.query({
      data: {
        ...
        variables: {
          id: `gid://shopify/InventoryItem/${response.inventory_id}`,
        },
      },
    });
    console.log(JSON.stringify(result));
    ...
    const invenLevelId =
      result.body.data.inventoryItem.inventoryLevels.edges[0].node.id;
